    ...
    : IInteractionAggregator
{
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            try
            {
                int savedChanges = await db.SaveChangesAsync(cancellationToken);
                logger.LogInformation("ETL completed. Saved {Count} changes to the database", savedChanges);
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Failed to save interaction scores to the database");
                throw;
            }
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "An error occurred during the interaction aggregation process");
            throw;
        }
    }
