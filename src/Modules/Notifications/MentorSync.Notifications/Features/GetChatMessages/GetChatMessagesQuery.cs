...
public sealed record GetChatMessagesQuery(string RoomId, int UserId) : IRequest<Result<List<GetChatMessagesResponse>>>;

public sealed class GetChatMessagesHandler(NotificationsDbContext dbContext) : IRequestHandler<GetChatMessagesQuery, Result<List<GetChatMessagesResponse>>>
{
    public async Task<Result<List<GetChatMessagesResponse>>> Handle(GetChatMessagesQuery request, CancellationToken cancellationToken)
    {
        ...
        var result = messages.Select(m => new GetChatMessagesResponse
        {
            Id = m.Id.ToString(),
            SenderId = m.SenderId,
