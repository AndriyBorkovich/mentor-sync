...
public class NotificationHub(NotificationsDbContext dbContext) : Hub
{
    ...
    public async Task SendChatMessage(int recipientId, string content)
    {
        ...
        await dbContext.ChatMessages.InsertOneAsync(message);
        ...
        await Clients.User(recipientId.ToString()).SendAsync("ReceiveChatMessage", messageDto);
    ...
    public async Task MarkMessageAsRead(string messageId)
    {
        ...
        var result = await dbContext.ChatMessages.UpdateOneAsync(filter, update);
    ...
    private async Task<ChatRoom> FindOrCreateChatRoom(int user1Id, int user2Id)
    {
        ...
        var existingRoom = await dbContext.ChatRooms.Find(filter).FirstOrDefaultAsync();
        ...
        await dbContext.ChatRooms.InsertOneAsync(newRoom);
        return newRoom;
    }
